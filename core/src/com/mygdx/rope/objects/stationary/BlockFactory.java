...
public class BlockFactory {
	...
	*/
	public BlockFactory(World world, FileHandle materialsFile, int loggingLevel) {
		logger = new Logger("MapBodyManager", loggingLevel);
		logger.info("initialising");
		
		this.world = world;
		this.units = Constants.TILES_SIZE;
        ...
        defaultFixture.filter.maskBits = Constants.MASK.get("Scenery");
        defaultFixture.filter.categoryBits = Constants.CATEGORY.get("Scenery");
...
//            filter.categoryBits = Constants.CATEGORY_SENSOR;
//            filter.maskBits = Constants.CATEGORY_SCENERY | Constants.CATEGORY_PLAYER;
	...
	*/
	...
	private void loadMaterialsFile(FileHandle materialsFile) {
		...
		try {
			...
			while (materialIt.hasNext()) {
				...
				fixtureDef.density = materialValue.getFloat("density", 1.0f); // 1.0f is the default value
	...
	private Shape getRectangle(RectangleMapObject rectangleObject) {
		...
		Vector2 size = new Vector2((rectangle.x + rectangle.width * 0.5f) / units, // 32/32 = 1
		(rectangle.y + rectangle.height * 0.5f ) / units); // seems to be the position
		polygon.setAsBox(rectangle.width * 0.5f / units,
		rectangle.height * 0.5f / units,size,0.0f);
		return polygon;
	}
	...
	private Shape getPolygon(PolygonMapObject polygonObject) {
		...
		float[] worldVertices = new float[vertices.length]; // translate in world unit
	...
	private Shape getPolyline(PolylineMapObject polylineObject) {
		...
		for (int i = 0; i < vertices.length / 2; ++i) {
			...
			worldVertices[i].x = vertices[i * 2] / units; // surely cause of a difference of data representation
